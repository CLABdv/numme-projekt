...
figure(3);
plot(xs, A);
legend("Maximal relativ ljudnivå i sovrummet", 'Location', 'southeast')
...
xs = 0.75;
fprintf("x0=%f är en relativt dålig placering\n", xs)
S = @(x, y) 1 * S0(x-xs, y-ys);
[B, Sol] = hhsolver(omega, S, 1000);
figure(6)
mesh(Sol.x,Sol.y,Sol.u)
...
for i = 1:5
    ...
    %{
    Ftemp = @(x0tilde, y0tilde, atilde, alphas) atilde .* eta .* vc(x0tilde, y0tilde, alphas)' - IcIntegral(alphas);
    F = @(X) Ftemp(X(1), X(2), X(3), alphas);
    J = @(X) JacIc(X(1), X(2), X(3), alphas, eta, omega);
